(function bootstrapReplyHistoryUi(root) {
  function upsertReplyMessage(replyMessages, payload) {
    ...
    if (existing) {
      ...
      existing.turnId = payload.turnId || existing.turnId || ''
      existing.sequence = Number(payload.sequence || existing.sequence || 0)
    } else {
  ...
  function attachReplyAudio(replyMessages, payload) {
    ...
    message.text = text
    message.kind = payload.kind || message.kind || ''
    message.turnId = payload.turnId || message.turnId || ''
    message.sequence = Number(payload.sequence || message.sequence || 0)
