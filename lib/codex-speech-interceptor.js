...
class CodexSpeechInterceptor {
  ...
  maybeFinalize(options = {}) {
    ...
    candidateLines = this.stripQueuedUserPromptBlockLines(candidateLines)
  ...
  stripQueuedUserPromptBlockLines(lines) {
    if (this.lastSubmittedInput || this.inputBuffer.trim() || this.visiblePromptDraft) {
      return lines
    }

    lines = this.stripLeadingQueuedPromptContinuationBlock(lines)

    const filtered = []
    let skippingQueuedPrompt = false

    for (const rawLine of lines) {
      const line = String(rawLine || '')
      const trimmed = line.trim()

      if (!skippingQueuedPrompt && this.isQueuedUserPromptLine(trimmed)) {
        skippingQueuedPrompt = true
        continue
      }

      if (!skippingQueuedPrompt) {
        filtered.push(rawLine)
        continue
      }

      if (!trimmed) {
        skippingQueuedPrompt = false
        continue
      }

      if (
        this.isPromptLine(trimmed) ||
        this.isFooterLine(trimmed) ||
        isSelectionMenuLine(trimmed) ||
        isStateCueLine(trimmed)
      ) {
        continue
      }
  ...
  stripLeadingQueuedPromptContinuationBlock(lines) {
    const firstBlankIndex = lines.findIndex((line) => !String(line || '').trim())

    if (firstBlankIndex < 2) {
      return lines
    }
    ...
    if (!/^[•◦●]\s+/.test(nextNonEmptyLine || '')) {
      return lines
    }

    const leadingBlock = lines.slice(0, firstBlankIndex).map((line) => String(line || '').trim())
    ...
    if (!leadingBlock.some((line) => /^[a-z0-9"'`(]/.test(line))) {
      return lines
    }

    if (leadingBlock.some((line) => /[.!?:]["')\]]*$/.test(line))) {
      return lines
    }

    return lines.slice(firstBlankIndex + 1)
  }

  isQueuedUserPromptLine(line) {
    if (!line) {
      return false
    }

    if (!/^(?:>|>>|›|»|❯)\s+\S/.test(line) && !/^(?:You:|User:|Human:|Prompt:)\s+\S/i.test(line)) {
      return false
    }

    if (
      this.isStoredPromptHintLine(line) ||
      this.isEchoedSubmittedInputLine(line) ||
      this.isEchoedDraftInputLine(line)
    ) {
      return false
    }
  ...
  capturePromptHint(text) {
    ...
    for (let index = lines.length - 1; index >= 0; index -= 1) {
      ...
      if (
        this.isCodexPrompt(rawLine) &&
        candidate &&
        !this.isLikelySubmittedPrompt(candidate) &&
        isPromptHintCandidate(candidate)
      ) {
...
function isPromptHintCandidate(text) {
  ...
  if (
    /^(?:npm|npx|node|pnpm|yarn|git|python|python3|pip|ls|cd|cat|grep|rg|find|sed|awk|chmod|chown|mv|cp|rm|mkdir|touch|curl|wget|docker|kubectl|bash|sh|pwsh|powershell)\b/i.test(
      candidate
    )
  ) {
    return false
  ...
  if ((candidate.match(/[\\/]/g) || []).length >= 2 && !/[.!?]$/.test(candidate)) {
    return false
  }
