...
function getSpeechTransitionKind(line) {
  ...
  if (
    /^[•◦●]\s*(?:ran\b|bash\b|read\s+\d+\s+files?\b|searched for\b|updated plan\b|working\b|running\b|waited for background terminal\b|tool loaded\b)/i.test(
      trimmed
    ) ||
...
const EMBEDDED_SPEECH_TRANSITION_PATTERNS = [
  {
    kind: 'tool',
    regex: /\s+[•◦●]\s+(?=(?:Working\b|Ran\b|Bash\b|Read\s+\d+\s+files?\b|Searched for\b|Updated Plan\b|Running\b|Waited for background terminal\b|Tool loaded\b))/i
  },
  {
    kind: 'approval',
