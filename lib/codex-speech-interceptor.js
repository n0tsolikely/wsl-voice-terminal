...
class CodexSpeechInterceptor {
  ...
  maybeFinalize() {
    ...
    while (candidateLines.length && this.isFooterLine(candidateLines[candidateLines.length - 1])) {
      candidateLines.pop()
    }
    ...
    if (!this.hasCompletionBoundary({ lines, lastLine, spokenText })) {
      return null
    }
  ...
  hasCompletionBoundary({ lines, lastLine, spokenText }) {
    if (!spokenText) {
      return false
    }
    ...
    if (this.activeAssistant === 'claude') {
      return this.hasClaudePromptReturn(lines)
    }

    if (this.isAssistantPrompt(lastLine)) {
      return true
    }

    return Boolean(this.sawAltScreenExit)
  }
