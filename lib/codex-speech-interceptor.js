...
class CodexSpeechInterceptor {
  constructor(onFinalizedText, options = {}) {
    ...
    this.idleMs = options.idleMs || 550
    this.reset()
  }

  reset() {
    this.codexSessionActive = false
    this.awaitingInteractiveCodex = false
  ...
  observeInput(data) {
    for (const char of data) {
      ...
      if (char === '\r' || char === '\n') {
        ...
        const isCodexCommand = /^codex(?:\s|$)/.test(submitted)
        const isOneShotCodexCommand = /^codex\s+exec(?:\s|$)/.test(submitted)

        if (isCodexCommand) {
          this.codexSessionActive = true
          this.awaitingInteractiveCodex = !isOneShotCodexCommand
        }

        if (this.codexSessionActive && !this.awaitingInteractiveCodex) {
  ...
  observeOutput(chunk) {
    ...
    if (this.looksLikeCodexSurface(plainChunk)) {
      this.codexSessionActive = true
      this.awaitingInteractiveCodex = false
    }

    if (this.awaitingInteractiveCodex) {
      return
    }
  ...
  maybeFinalize() {
    ...
    while (candidateLines.length && this.isCodexFooterLine(candidateLines[candidateLines.length - 1])) {
      candidateLines.pop()
    }

    const lastLine = getLastNonEmptyLine(candidateLines)
