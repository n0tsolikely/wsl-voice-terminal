...
function extractStateCueText(text) {
  const lines = normalizeTerminalText(text)
    .split('\n')
    .map((line) => sanitizeStateCueLine(line))
    .filter(Boolean)
...
function isSelectionMenuLine(line) {
  const trimmed = String(line || '').trim()
  ...
  return (
    /^(?:select|choose)\s+(?:model|reasoning|effort|provider|profile|mode)\b/i.test(trimmed) ||
    /^access\s+legacy\s+models\b/i.test(trimmed) ||
    /^use\s+(?:the\s+)?arrow\s+keys\b/i.test(trimmed) ||
    /^press\s+enter\s+to\s+(?:choose|select)\b/i.test(trimmed) ||
    /^\d+\.\s+(?:(?:gpt|o\d|claude|gemini|codex)[\w .:-]*|(?:low|medium|high|xhigh|max|minimal|balanced|fast))$/i.test(
      trimmed
    ) ||
    /^\d+\.\s+(?:switch\s+to|keep\s+current)\b/i.test(trimmed)
  )
}

function isStateCueLine(line) {
  const trimmed = String(line || '').trim()
  ...
  return /^(?:model|reasoning(?:\s+level)?|effort|provider|directory)\s+changed\s+to\b/i.test(
    trimmed
  )
}

function sanitizeStateCueLine(line) {
  const trimmed = stripAssistantChromePrefix(stripMergedFooterChrome(String(line || '').trim()))

  if (!trimmed || !isStateCueLine(trimmed)) {
    return ''
  }
...
function shouldIgnoreSpeechLine(line) {
  ...
  if (/^messages to be submitted after next tool call\b/i.test(line)) {
    return true
  }
  ...
  if (/\/ps to view\b/i.test(line) || /\/stop to close\b/i.test(line)) {
    return true
  }
  ...
  if (isSelectionMenuLine(line) || isStateCueLine(line)) {
    return true
  }

  if (isContextMeterLine(line)) {
    return true
  }

  if (
    /^(?:model|engine|provider)\s*:/i.test(line) ||
    /^(?:gpt|o\d|claude|gemini|codex)[\w .:-]*\d+%\s+left$/i.test(line) ||
...
function isContextMeterLine(line) {
  const trimmed = String(line || '').trim()
  ...
  if (/\bcontext left\b/i.test(trimmed)) {
    return true
  }

  if (!/\b\d+%\s+left\b/i.test(trimmed)) {
    return false
  }

  return (
    /[\\/]/.test(trimmed) ||
    /^(?:(?:OpenAI\s+)?codex\b|(?:gpt|o\d|claude|gemini|codex)\b)/i.test(trimmed)
  )
}
...
function isToolProgressLine(line) {
  ...
  return /^(?:explor(?:e|ed|ing)|working|ran|running|read(?:ing)?(?:\s+\d+\s+files?)?|search(?:ed(?:\s+for)?|ing)?|list(?:ed|ing)?|open(?:ed|ing)?|inspect(?:ed|ing)?|review(?:ed|ing)?|analyz(?:e|ed|ing)|patch(?:ed|ing)|apply(?:ing)?\s+patch|applypatch|update(?:d)?\s+plan|waited\s+for\s+background\s+terminal|tool\s+loaded|bash)\b/i.test(
    normalized
  )
}
