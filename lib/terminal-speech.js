...
function isToolProgressLine(line) {
  ...
  return /^(?:explor(?:e|ed|ing)|working|ran|running|read(?:ing)?(?:\s+\d+\s+files?)?|search(?:ed(?:\s+for)?|ing)?|open(?:ed|ing)?|inspect(?:ed|ing)?|review(?:ed|ing)?|analyz(?:e|ed|ing)|patch(?:ed|ing)|apply(?:ing)?\s+patch|applypatch|update(?:d)?\s+plan|waited\s+for\s+background\s+terminal|tool\s+loaded|bash)\b/i.test(
    normalized
  )
}
