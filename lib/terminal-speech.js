...
function extractSpeechText(text) {
  const paragraphs = stripCodeBlocks(text)
    .split(/\n{2,}/)
    .map((paragraph, index) => {
      if (isUserPromptParagraph(paragraph)) {
        return {
          index,
          paragraph: '',
          score: 0
  ...
  const rankedRuns = runs
    .map((run) => ({
      run,
      totalScore: run.reduce((sum, paragraph) => sum + paragraph.score, 0),
      endIndex: run[run.length - 1].index
    }))
    .sort(
      (left, right) =>
        right.totalScore + right.endIndex * 3 - (left.totalScore + left.endIndex * 3)
    )

  return rankedRuns[0].run
...
function shouldIgnoreSpeechLine(line) {
  ...
  if (/^esc to cancel\b/i.test(line)) {
    return true
  }
