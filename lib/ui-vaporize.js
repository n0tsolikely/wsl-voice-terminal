(function attachUiVaporize(globalScope) {
  ...
  function vaporizeElement(element, options = {}) {
    ...
    return renderSnapshotToCanvas(snapshotProfile.clone, rect)
      .catch(() => renderFallbackSnapshot(snapshotProfile, rect))
      .then((canvas) => {
        if (!canvas) {
          return false
        }
        ...
        return animateParticles(overlayCanvas, particleState, particleOptions, canvas).then(() => true)
      })
      .catch(() => false)
  }
  ...
  function animateParticles(canvas, particles, options, snapshotCanvas) {
    const context = canvas.getContext('2d')
    ...
    return new Promise((resolve) => {
      function frame(now) {
        ...
        if (snapshotCanvas && progress < 0.42) {
          const ghostFade = Math.max(0, 1 - progress / 0.42)
          const ghostLift = progress * -8
          const ghostScale = 1 + progress * 0.018

          context.save()
          context.globalAlpha = ghostFade * 0.92
          context.filter = `blur(${(1 - ghostFade) * 3.4}px)`
          context.translate(margin, margin + ghostLift)
          context.scale(ghostScale, ghostScale)
          context.drawImage(
            snapshotCanvas,
            0,
            0,
            snapshotCanvas.width / pixelRatio,
            snapshotCanvas.height / pixelRatio
          )
          context.restore()
        }
        ...
        for (const particle of particles) {
          ...
          if (particle.size <= 3.2 && fade > 0.18) {
            context.globalAlpha = fade * 0.22
            context.fillStyle = '#ffffff'
            context.beginPath()
            context.arc(size * 0.5, size * 0.5, Math.max(0.5, size * 0.32), 0, Math.PI * 2)
            context.fill()
          }
