(function attachUiVaporize(globalScope) {
  ...
  function vaporizeElement(element, options = {}) {
    ...
    return renderSnapshotToCanvas(snapshotProfile.clone, rect)
      ...
      .catch(() =>
        renderFallbackSnapshot(snapshotProfile, rect).then((canvas) => ({
          canvas,
          method: 'fallback'
        }))
      )
      .then((canvas) => {
        ...
        if (!particleState.length && renderMethod === 'svg') {
          return renderFallbackSnapshot(snapshotProfile, rect).then((fallbackCanvas) => {
            particleState = createParticleState(fallbackCanvas, rect, particleOptions)
            renderMethod = 'fallback-blank-svg'
